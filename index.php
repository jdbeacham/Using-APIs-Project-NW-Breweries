...
<div id="navWrapper">
    <a href="https://jb-codes.com/api/">
    <div id="icon"><svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
...
<div class="navButtons"><a href="?state=Idaho">Idaho</a></div>
<div class="navButtons"><a href="?state=Oregon">Oregon</a></div>
<div class="navButtons"><a href="?state=Washington">Washington</a></div>
</div>
<center>
...
if (isset($location) || $myLocation=="yes" || isset($listCountNew)) {
    ...
    $brewery_list = $_SESSION['placeList'];
...
if (count($brewery_list[$listCount]) == 1) {
    $listTitle = "1 Brewery";
}
    else {
if ($listCount == 0) {
    $listTitle = count($brewery_list[$listCount]) . " Breweries";
if (count($brewery_list[$listCount]) == 0) {
    $listTitle = "Zero Breweries Found. Try Again!";
}
}
if ($listCount < 4 && $listCount > 0) {
    $word = $wordArray[$random];
    $listTitle = count($brewery_list[$listCount]) . " " . $word . " Breweries";
...
for ($i = 0; $i < count($brewery_list[$listCount]); $i++) {
        echo  '<div class = "listingWrapper">
    <div style="font-size: 1.5em; color: seagreen;">' .$brewery_list[$listCount][$i]["name"] . '</div>
    <div>' . $brewery_list[$listCount][$i]["street"] . ' | ' . $brewery_list[$listCount][$i]["city"]
    . ', ' . $brewery_list[$listCount][$i]["state"] . '</div>
    <div class="inlineList">' .
    $brewery_list[$listCount][$i]["distance"] . '</div> &#9672; ';
        if (isset($brewery_list[$listCount][$i]["website_url"])) {
        echo '<div class="inlineList" id="inlineListLink"><a href="' . $brewery_list[$listCount][$i]["website_url"] . 
        '" target="_blank" rel="noopener noreferrer">website</a></div> &#9672; ';
        }
        echo '<div class="inlineList" 
        id="directionsListLink"><a href="https://www.google.com/maps/search/?api=1&query=' . $brewery_list[$listCount][$i]["street"] . '+' . $brewery_list[$listCount][$i]["city"]
        . '+' . $brewery_list[$listCount][$i]["state"] .
...
for ($i = 1; $i <= count($brewery_list); $i++) {
...
<div class="subTitle">Explore Beer in Washington, Oregon, Idaho</div>
